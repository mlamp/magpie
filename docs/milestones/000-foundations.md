...
- [x] Cargo workspace at repo root with member crates:
  - [x] `magpie-bt-bencode` — zero-copy decode/encode landed in Phase A (strict canonical, typed errors, proptest, fuzz target + seeds, criterion baseline).
  - [x] `magpie-bt-metainfo` — v1/v2/hybrid parser landed in Phase B; `InfoHash::{V1, V2, Hybrid}` computed from raw info-dict span (sha1 / sha256); zero-copy `MetaInfo<'a>`; proptest + fuzz + bench baseline.
  - [x] `magpie-bt-wire` — placeholder; peer wire protocol codec lands in M1.
  - [x] `magpie-bt-core` — peer-ID builder, custom alert ring (ADR-0002), `Storage` trait + memory/file impls, rarest-first piece picker with endgame landed in Phase C.
...
- [x] `Storage` trait + file-backed impl (scalar; uses stdlib `FileExt` rather than raw `pwritev`/`preadv` — vectorised variant deferred to ADR-0004 post-M0) + in-memory impl.
- [x] Piece picker skeleton: rarest-first + endgame mode, no network wiring (Phase C).
- [x] Event bus: custom double-buffered alert ring per ADR-0002 (Phase C). Earlier proposal of `tokio::sync::broadcast<TorrentEvent>` superseded.
- [x] Azureus-style peer-ID builder: `client_code: [u8; 2]`, `version: [u8; 4]`, 12-byte random suffix (Phase C).
- [x] v2 invariant enforcement in v1 paths: 16 KiB blocks, power-of-two piece sizes (Phase B, `ParseErrorKind::InvalidPieceLength`).
- [x] Typed errors per module using `thiserror`.

### Quality deliverables (per DISCIPLINES.md)

- [ ] **Property tests** (`proptest`) for `-bencode` round-trip (done — Phase A), `-metainfo` parse/info-hash invariants (done — Phase B), picker on synthetic bitfields.
- [ ] **Fuzz targets** (`cargo-fuzz`): `bencode_decode` (done — Phase A), `metainfo_parse` (done — Phase B; wired to info-hash verification assertion, seeds for v1/v2 under `crates/magpie-bt-metainfo/fuzz/corpus/metainfo/`). Nightly wiring remains for M0 close.
...
- [x] **Docs**: every `pub` item has a rustdoc summary; each crate root has an intro + runnable example; `cargo doc -D warnings` passes.
- [x] **CHANGELOG** under `## [Unreleased]` lists every public-API addition.
