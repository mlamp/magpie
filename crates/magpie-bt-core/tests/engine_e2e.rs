...
async fn engine_fetches_synthetic_torrent_from_three_tcp_seeders() {
    ...
    // Wait for completion alert (or timeout). 15 s (not 5 s) because
    // this test spins 3 mock seeders + 1 engine leech on one runtime;
    // under `--all-features` full-workspace parallel load the 5 s
    // budget occasionally slips CPU-bound. See task #25.
